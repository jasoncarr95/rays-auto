...
jQuery(document).ready(function ($) {
  ...
  var preloader = function () {
    $(
      ".blog a.post-image>img, .blog:not('.horizontal-carousel').three-columns li>a>img, .post.single .post-image img, .services-list a>img, .team-box img, .galleries-list:not('.isotope') a>img, .re-preload>img"
    ).each(function () {
  ...
  $(".our-clients-list:not('.type-list')").each(function (index) {
    ...
    $(".re-preloader_" + index).imagesLoaded(function () {
      ...
      self.carouFredSel({
        items: {
          visible:
            $(".header").width() > 750 ? 6 : $(".header").width() > 462 ? 4 : 2,
        },
        scroll: {
          items:
            $(".header").width() > 750 ? 6 : $(".header").width() > 462 ? 4 : 2,
  ...
  $(window)
    .on("hashchange", function (event) {
      ...
      if (hashSplit[0].substr(0, 11) != "prettyPhoto") {
        ...
        $(".testimonials-list, .our-clients-list").trigger("configuration", [
          "debug",
          false,
          true,
        ]);
  ...
  //hint
  $(
    ".comment-form input[type='text'], .contact-form input[type='text']:not('.cost-slider-datepicker'), .comment-form textarea, .contact-form textarea, .search input[type='text'], .search-form input[type='text'], .cost-calculator-container input[placeholder]:not('.cost-slider-datepicker')"
  ).hint();
